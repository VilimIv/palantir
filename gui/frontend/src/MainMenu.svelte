<script>
  ...
  function onOrbClick() {
    orbClicks++
    clearTimeout(timer)
    timer = setTimeout(() => { orbClicks = 0 }, 1200)
...
<style>
  ...
  .eye-outer{width:180px;height:80px;border-radius:50%;background:radial-gradient(ellipse,#ff4500,#bb1100 40%,#550000 75%,#1a0000);box-shadow:0 0 80px rgba(255,69,0,.8),0 0 160px rgba(255,30,0,.4);position:relative;overflow:hidden;animation:eP .6s ease-in-out infinite}
  ...
  .mini-eye{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:70%;height:40%;border-radius:50%;background:radial-gradient(ellipse,#ff6600,#cc3300,transparent);animation:mE .3s ease-out}
  @keyframes mE{from{height:5%;opacity:0}to{height:40%;opacity:1}}
  ...
  .title{font-family:'Cinzel','Georgia',serif;font-size:2.5em;color:#c9a84c;letter-spacing:4px;text-shadow:0 0 25px rgba(201,168,76,.25),0 2px 4px rgba(0,0,0,.6)}
  ...
  @keyframes fl{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px);box-shadow:0 0 35px rgba(40,90,130,.6)}}
