      ...
      {#each peers as p}
        <div class="peer">
          ...
          <div class="pr">
            ...
            {#if !p.ready}
              <span class="mode wait">···</span>
            {:else if !p.dataConfirmed}
              <span class="mode pending" title="Handshake ok, čekam podatke">{p.mode}?</span>
            {:else}
...
<style>
  ...
  .mode.wait{background:#1a1a14;color:#6b7b4a;border:1px solid #2a3d2a;animation:blink 1.5s infinite}
  .mode.pending{background:#1a2020;color:#6a8a8a;border:1px solid #2a4040;font-style:italic}
